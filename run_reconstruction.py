...
if __name__ == "__main__":
    ...
    # Add path_to_events to args for ImageWriter to use
    args.path_to_events = path_to_events

    # Try to read header, handle different file formats
    try:
        header = pd.read_csv(
            path_to_events,
            sep=r"\s+",
        ...
        print(f"Warning: Could not parse header from {path_to_events}")
        print(f"Error: {e}")
        print("Using default resolution 346x260 for EVSR dataset")
    ...
    if args.fixed_duration:
        event_window_iterator = FixedDurationEventReader(
            path_to_events, duration_ms=args.window_duration, start_index=start_index
        )
    else:
