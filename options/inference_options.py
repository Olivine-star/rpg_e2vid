def set_inference_options(parser):
    parser.add_argument(
        "-o", "--output_folder", default=None, type=str
    )  # if None, will not write the images to disk
    parser.add_argument("--dataset_name", default="reconstruction", type=str)
    parser.add_argument(
        "--use_event_path_for_output",
        dest="use_event_path_for_output",
        action="store_true",
        help="Save output images in the same directory as the input event file",
    )
    parser.set_defaults(use_event_path_for_output=False)
