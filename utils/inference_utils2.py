...
class ImageDisplay:
    """
    Utility class to display and optionally save image reconstructions + events
    """
    ...
    def __init__(self, options):
        ...
        # Output directory for saving frames and videos
        self.output_dir = getattr(options, "output_dir", "output")
        os.makedirs(os.path.join(self.output_dir, "frames"), exist_ok=True)

        # Get resolution
        self.width = options.width
        self.height = options.height
        self.frame_size = (self.width * 2, self.height)  # side-by-side

        # Video writer
        self.video_writer = cv2.VideoWriter(
        os.path.join(self.output_dir, "reconstruction.mp4"),
    ...
    def __call__(self, img, events=None):
        ...
        if self.show_events and events is not None:
            event_preview = make_event_preview(events, mode=self.event_display_mode,
                                               num_bins_to_show=self.num_bins_to_show)
            event_preview = self.crop_outer_border(event_preview, self.border)
        else:
            event_preview = np.zeros_like(img)

        # Convert both to color if needed
        if len(img.shape) == 2:
            img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        if len(event_preview.shape) == 2:
            event_preview = cv2.cvtColor(event_preview, cv2.COLOR_GRAY2BGR)

        # Concatenate side-by-side
        combined = np.hstack([event_preview, img])

        # Save to disk as image
        frame_path = os.path.join(self.output_dir, "frames", f"frame_{self.frame_id:05d}.png")
        cv2.imwrite(frame_path, combined)

        # Save to video
        self.video_writer.write(combined)

        # Show in window if needed
        if self.display:
