...
class ImageWriter:
    ...
    def __init__(self, options):
        ...
        # Check if we should use event file path for output directory
        self.use_event_path = getattr(options, "use_event_path_for_output", False)
        self.event_file_path = getattr(options, "path_to_events", None)

        print("== Image Writer ==")
        if self.output_folder:
            # If use_event_path is enabled and event file path is provided
            if self.use_event_path and self.event_file_path:
                # Extract directory from event file path
                import os

                event_dir = os.path.dirname(self.event_file_path)
                # Create output directory in the same folder as the event file
                self.final_output_dir = join(event_dir, self.dataset_name)
                print(
                    "Using event file directory for output: {}".format(
                        self.final_output_dir
                    )
                )
            else:
                # Use the original logic
                self.final_output_dir = join(self.output_folder, self.dataset_name)
                print("Using specified output folder: {}".format(self.final_output_dir))

            ensure_dir(self.final_output_dir)
            print("Will write images to: {}".format(self.final_output_dir))
            self.timestamps_file = open(
                join(self.final_output_dir, "timestamps.txt"), "a"
            )

            if self.save_events:
                self.event_previews_folder = join(self.final_output_dir, "events")
